...
│                                                                              │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
│  │   TimescaleDB   │  │   ClickHouse    │  │ Elasticsearch   │             │
│  │  (时序存储)     │  │  (OLAP 分析)    │  │  (全文检索)    │             │
│  │                 │  │                 │  │                 │             │
│  │ • 原始日志     │  │ • 聚合统计     │  │ • 事件搜索     │             │
│  │ • 指标数据     │  │ • 报表查询     │  │ • 日志分析     │             │
...
| 组件 | 用途 | 特点 |
|------|------|------|
| **TimescaleDB** | 时序存储 | 高写入时序数据、压缩、自动分区 |
| **ClickHouse** | OLAP 分析 | 高性能分析查询、列式存储 |
| **Elasticsearch** | 全文检索 | 日志搜索、IOC 匹配 |
| **Redis** | 缓存 | 热数据缓存、会话、限流 |
| **PostgreSQL** | 配置存储 | 用户、规则、资产配置 |
...
| ClickHouse | 4 vCPU, 16GB RAM, 500GB | ¥1,000 |
| Redis | 4GB | ¥300 |
| PostgreSQL | 4 vCPU, 8GB RAM, 200GB | ¥500 |
| Elasticsearch | 2 节点, 200GB | ¥1,200 |
| **合计** | | **¥7,300/月** |
...
| ClickHouse | 3 节点集群, 1TB | ¥2,500 |
| Elasticsearch | 5 节点集群, 1TB | ¥3,000 |
...
│  ┌─────────────────────────────────────────────────┐   │
│  │              存储层 (分布式)                       │   │
│  │  TimescaleDB │ ClickHouse │ Elasticsearch │ Redis│   │
│  └─────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```
